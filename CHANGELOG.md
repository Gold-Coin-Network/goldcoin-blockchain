...
- Correctly display private key in `goldcoin keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `goldcoin keys show`.
...
- We added a simple profiler to track performance times in the application (see /goldcoin/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `goldcoin configure -t true|false`).
...
- Secret wallet key is hidden by default in `goldcoin keys show`. Use `goldcoin keys show --show-mnemonic-seed` for private keys.
...
- The `src` directory in goldcoin-blockchain has been changed to `goldcoin` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `goldcoin/`
...
- @jespino changed `goldcoin keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `goldcoin keys show`.
...
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `goldcoin keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/goldcoin-blockchain/).
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/goldcoin-blockchain).
...
- Migration with GOLDCOIN_ROOT set does not crash goldcoin init
...
- `goldcoin farm summary` displayed some incorrect amounts.
...
- `goldcoin netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `goldcoin netspace`. Thanks @altendky.
- There was a typo in `goldcoin plots`. Thanks @adamfiddler.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `ozt_target_address` sections of config.yaml.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of goldcoin-blockchain, etc., instead of in blspy.
...
- Adding Click changed the behavior of `goldcoin keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
...
- Interface improvements for `goldcoin netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `goldcoin init` only migrates release candidate directories. The versioned sub directories under `~/goldcoin` will be going away before mainnet.
...
- `goldcoin configure -log-level` was broken.
...
- `goldcoin plots check -l` will list all duplicate plot IDs found on the machine. Thanks very much for this PR @eFishCent.
...
- `goldcoin plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- `goldcoin configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `goldcoin plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `goldcoin plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `goldcoin show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `goldcoin show` handles sub block 0 better.
...
- `goldcoin plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `goldcoin plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `goldcoin show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `goldcoin configure --enable-upnp`.
...
- `goldcoin configure` now lets you enable or disable uPnP.
...
- `goldcoin netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `goldcoin show` of the GUI use `goldcoin netspace -d 1000` as `goldcoin netspace` defaults to `-d 192` which is one hour.
...
- `goldcoin show -s` now is less brain dead when a node is initially starting to sync.
...
- The rpc interfaces of all goldcoin services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test goldcoin since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve goldcoin-blockgoldcoinn's Pypi entry. Thanks to @altendky for this pull request.
...
- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test goldcoin are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.goldcoin` directory.
...
- `goldcoin show -w` should now more reliably work. Wallet balances should be more often correct.
...
- `goldcoin show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- See wallet balances in command line: `goldcoin show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `goldcoin keys sign` and `goldcoin keys verify` to allow farmers to certify their ownership of keys.
...
- `goldcoin start wallet-server` changed to `goldcoin start wallet`, for consistency.
...
- Removed legacy scripts such as goldcoin-stop-server, goldcoin-restart-harvester, etc.
...
"goldcoin plots create" and "goldcoin plots check".
- Add plot directories to config.yaml from the cli with "goldcoin plots add".
...
- Starting goldcoin now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'goldcoin keys' command line.
...
- We added a `goldcoin keys` command line program, to see, add, and remove private keys.
...
- `goldcoin netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `goldcoin-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `goldcoin-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `goldcoin init` then sync on first start will happen automatically. "\$GOLDCOIN_ROOT" users will need to delete `$GOLDCOIN_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `goldcoin netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
...
- The script `goldcoin start wallet-gui` has been chaned to `goldcoin start wallet` which launches but the GUI and server on MacOS and Linux. `goldcoin start wallet-server` remains for WSL 2 and Windows native.
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `goldcoin stop all` command sometimes fails, use `goldcoin-stop-all` instead. In windows, use the task manager to stop the servers.
...
- We added full block lookup to `goldcoin show`.

### Changed

- `goldcoin-restart-harvester` has been renamed from `goldcoin-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.goldcoin/beta-1.0b3/config/plots.yaml` after migrating
...
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to goldcoin_full node, goldcoin_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new goldcoin services were started.
