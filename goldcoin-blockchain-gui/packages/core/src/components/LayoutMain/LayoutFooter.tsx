...
async function openFAQURL(): Promise<void> {
  try {
    ...
    await shell.openExternal('https://github.com/Goldcoin-Network/goldcoin-blockchain/wiki/FAQ');
  }
  catch (e) {
    console.error(e);
  }
...
async function openSendFeedbackURL(): Promise<void> {
  try {
    const shell: Shell = (window as any).shell;
    await shell.openExternal('https://feedback.goldcoin-network.net/lightwallet');
  }
  catch (e) {
    console.error(e);
  }
