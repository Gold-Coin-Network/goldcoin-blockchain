...
import { useGetBlockQuery, useGetBlockRecordQuery  } from '@goldcoin/api-react'
...
export default function Block() {
  ...
  const poolReward = mojoToGoldcoin(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToGoldcoin(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const goldcoinFees = blockRecord.fees !== undefined
    ? mojoToGoldcoin(blockRecord.fees)
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.goldcoinexplorer.com/blockchain/puzzlehash/${blockRecord.farmerPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.goldcoinexplorer.com/blockchain/puzzlehash/${blockRecord.poolPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: goldcoinFees ? `${goldcoinFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <>
      ...
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Goldcoin blockchain
            </Trans>
          </Back>
        }
        action={
