...
import goldcoinEnvironment from '../util/goldcoinEnvironment';
import { i18n } from '../config/locales';
import About from '../components/about/About';
import packageJson from '../../package.json';
import AppIcon from '../assets/img/goldcoin64x64.png';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!goldcoinEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
      app.quit();
      return false;
    }
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    const createWindow = async () => {
      if (manageDaemonLifetime(NET)) {
        goldcoinEnvironment.startGoldcoinDaemon();
      }
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Earn money while farming Goldcoin & Forks' }),
            click: () => {
              openExternal(
                'https://earnapp.com/i/e8ljhze',
              );
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Goldcoin Github repository' }),
            click: () => {
              openExternal(
                'https://github.com/Gold-Coin-Network/goldcoin-blockchain',
              );
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Goldcoin project' }),
            click: () => {
              openExternal(
          ...
          {
            label: i18n._(/* i18n */ { id: 'Join our community' }),
            click: () => {
              openExternal(
          ...
          {
            label: i18n._(/* i18n */ { id: 'Buy OZT on Forks.green' }),
            click: () => {
              openExternal(
                'https://www.forks.green/trading/oztxch',
              );
            },
          },
        ],
    ...
    if (process.platform === 'darwin') {
      // Goldcoin Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Goldcoin' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Goldcoin Blockchain' }),
            click: () => {
              openAbout();
            },
          },
