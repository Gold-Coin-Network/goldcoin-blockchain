...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToGoldcoinLocaleString, mojoToCATLocaleString } from '@goldcoin/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@goldcoin/api-react';
import styled from 'styled-components';
import type { Row } from '@goldcoin/core';
import { WalletType, TransactionType } from '@goldcoin/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToGoldcoinLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToGoldcoinLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
