...
import { AlertDialog, Card, Flex, Loading, Dropzone, mojoToGoldcoinLocaleString } from '@goldcoin/core';
import {
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToGoldcoinLocaleString(props.balance)} TOZT
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const CreateAttest = (props) => {
  ...
  function createAttestPacket() {
    ...
    if (address.substring(0, 12) === 'goldcoin_addr://') {
...
const CashoutCard = (props) => {
  ...
  function cashout() {
    ...
    if (puzzlehash.slice(0, 12) === 'goldcoin_addr://') {
