...
import { useCreateOfferForIdsMutation } from '@goldcoin/api-react';
import {
  Button,
  Divider,
  Grid,
...
import { goldcoinToMojo, catToMojo } from '@goldcoin/core';
import fs from 'fs';
  ...
  function updateOffer(offer: { [key: string]: number | string }, row: OfferEditorRowData, debit: boolean) {
    ...
    if (assetWalletId > 0) {
      ...
      if (walletType === WalletType.STANDARD_WALLET) {
        mojoAmount = Number.parseFloat(goldcoinToMojo(amount));
      }
