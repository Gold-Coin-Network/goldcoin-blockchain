...
import { OfferTradeRecord } from '@goldcoin/api';
import fs from 'fs';
...
import { Trade as TradeIcon } from '@goldcoin/icons';
import { useCancelOfferMutation, useGetOfferDataMutation, useGetWalletsQuery } from '@goldcoin/api-react';
...
function ConfirmOfferCancellation(props: ConfirmOfferCancellationProps) {
  ...
  async function handleConfirm() {
    const { fee: oztFee } = methods.getValues();
    var fee = 0;

    if (cancelWithTransaction) {
      fee = Number.parseFloat(goldcoinToMojo(oztFee));
    }
      ...
      <Grid container>
        <Grid xs={12} md={6} lg={5} item>
          <CardHero>
            ...
            <Typography variant="body1">
              <Trans>
                Create an offer to exchange OZT or other tokens. View an offer to inspect and accept an offer made by another party.
              </Trans>
            </Typography>
