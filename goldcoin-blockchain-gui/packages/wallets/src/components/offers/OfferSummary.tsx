...
import { type OfferSummaryRecord  } from '@goldcoin/api';
import {
  Flex,
  StateColor,
  mojoToGoldcoin,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries[0][0].toLowerCase() === 'ozt' ? Number(mojoToGoldcoin(makerEntries[0][1])) : Number(mojoToCAT(makerEntries[0][1]));
  const takerAmount = takerEntries[0][0].toLowerCase() === 'ozt' ? Number(mojoToGoldcoin(takerEntries[0][1])) : Number(mojoToCAT(takerEntries[0][1]));
