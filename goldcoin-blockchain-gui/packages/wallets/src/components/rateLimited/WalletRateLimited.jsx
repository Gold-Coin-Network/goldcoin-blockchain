...
import { AlertDialog, Card, Flex, goldcoinToMojo, mojoToGoldcoinLocaleString } from '@goldcoin/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const heozteck = /[\da-f]+$/gi;

    if (!heozteck.test(ip_val) || ip_val.value === '') {
    ...
    const goldcoinper_input = ip_parsed.limit;
    ...
    const goldcoinper_value = Number.parseInt(Number(goldcoinper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (goldcoin per interval):{' '}
                  {mojoToGoldcoinLocaleString(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToGoldcoinLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = goldcoinToMojo(amount_input.value);
    const fee = goldcoinToMojo(fee_input.value);
