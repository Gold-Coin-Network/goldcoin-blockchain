...
export default function Block() {
  ...
  const poolReward = mojo_to_goldcoin(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_goldcoin(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const goldcoinFees = blockRecord.fees
    ? mojo_to_goldcoin(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/goldcoin/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/goldcoin/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
