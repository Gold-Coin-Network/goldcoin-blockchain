...
import { AlertDialog } from '@goldcoin/core';
import {
  Typography,
  Button,
  Box,
...
import { goldcoin_to_mojo } from '../../../util/goldcoin';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      goldcoinper_input.value === '' ||
      Number(goldcoinper_input.value) === 0 ||
      !Number(goldcoinper_input.value) ||
      isNaN(Number(goldcoinper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const goldcoinper = goldcoin_to_mojo(goldcoinper_input.value);
    const goldcoinper_value = Number.parseInt(Number(goldcoinper));
    const userpubkey = userpubkey_input.value;
    const amount = goldcoin_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = goldcoin_to_mojo(fee_input.value);
