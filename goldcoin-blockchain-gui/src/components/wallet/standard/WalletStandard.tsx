...
import {
  ...
  Fee,
  Form,
  TextField as GoldcoinTextField,
  AlertDialog,
  CopyToClipboard,
  Flex,
  Card,
...
import { /* mojo_to_goldcoin_string, */ goldcoin_to_mojo } from '../../../util/goldcoin';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_goldcoin_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of goldcoin in the blockchain at the current
      ...
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the amount of goldcoin that you can currently use to make
            transactions. It does not include pending farming rewards, pending
            incoming transactions, and goldcoin that you have just spent but is not
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send goldcoin to coloured address. Please enter a goldcoin
              address.
            </Trans>
          </AlertDialog>,
        ),
    ...
    if (address.slice(0, 12) === 'goldcoin_addr://') {
    ...
    const amountValue = Number.parseFloat(goldcoin_to_mojo(amount));
    const feeValue = Number.parseFloat(goldcoin_to_mojo(fee));
