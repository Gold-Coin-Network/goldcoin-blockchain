...
global.daemon_rpc_ws = `wss://${self_hostname}:16322`;
...
function loadConfig(net) {
  try {
    // check if goldcoin_ROOT is set. it overrides 'net'
    const config_root_dir =
      'goldcoin_ROOT' in process.env
        ? process.env.goldcoin_ROOT
        : path.join(os.homedir(), '.goldcoin', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 16322); // jshint ignore:line
