...
const PY_FOLDER = '../goldcoin/daemon';
...
const getgoldcoinVersion = () => {
  let version = null;
  const exePath = getExecutablePath('goldcoin');
  // first see if we can get a goldcoin exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or goldcoin is on the path
    try {
      version = child_process
