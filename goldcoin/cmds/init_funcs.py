...
from goldcoin.ssl.create_ssl import generate_ca_signed_cert, get_goldcoin_ca_crt_key, make_ca_cert
...
from goldcoin.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("ozt_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("ozt_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "ozt_target_address" not in config["farmer"]:
        print(f"Setting the ozt destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "ozt_target_address" not in config["pool"]:
        print(f"Setting the ozt destination address for coinbase reward to {all_targets[0]}")
...
def create_all_ssl(root: Path):
    ...
    goldcoin_ca_crt_path.write_bytes(goldcoin_ca_crt)
    goldcoin_ca_key_path.write_bytes(goldcoin_ca_key)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if goldcoin_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return goldcoin_init(root_path)
...
def goldcoin_init(root_path: Path):
