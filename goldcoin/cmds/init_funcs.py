...
from goldcoin.consensus.coinbase import create_puzzlehash_for_pk
from goldcoin.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_goldcoin_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from goldcoin.util.bech32m import encode_puzzle_hash
from goldcoin.util.config import (
    create_default_goldcoin_config,
    initial_config_file,
    load_config,
    save_config,
...
from goldcoin.util.ssl_check import (
...
from goldcoin.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'goldcoin keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "ozt_target_address" not in config["farmer"]
    stop_searching_for_pool = "ozt_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("ozt_target_address") or all_targets[-2] == config["farmer"].get(
                "ozt_target_address"
            ):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("ozt_target_address") or all_targets[-2] == config["pool"].get(
                "ozt_target_address"
            ):
    ...
    if "ozt_target_address" not in config["farmer"]:
        print(
            f"Setting the ozt destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["ozt_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["ozt_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we might not have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['ozt_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "ozt_target_address" not in config["pool"]:
        print(f"Setting the ozt destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["ozt_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["ozt_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we might not have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['ozt_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the OZT destination addresses, edit the `ozt_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    goldcoin_ca_crt, goldcoin_ca_key = get_goldcoin_ca_crt_key()
    goldcoin_ca_crt_path = ca_dir / "goldcoin_ca.crt"
    goldcoin_ca_key_path = ca_dir / "goldcoin_ca.key"
    write_ssl_cert_and_key(goldcoin_ca_crt_path, goldcoin_ca_crt, goldcoin_ca_key_path, goldcoin_ca_key)
    ...
    goldcoin_ca_crt, goldcoin_ca_key = get_goldcoin_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, goldcoin_ca_crt, goldcoin_ca_key, False, overwrite=False)
...
def init(
    ...
    if create_certs is not None:
        ...
        return goldcoin_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)


def goldcoin_version_number() -> Tuple[str, str, str, str]:
...
def goldcoin_minor_release_number():
    res = int(goldcoin_version_number()[2])
    print(f"Install release number: {res}")
    return res


def goldcoin_full_version_str() -> str:
    major, minor, patch, dev = goldcoin_version_number()
...
def goldcoin_init(
    ...
    *,
    ...
    """
    if os.environ.get("GOLDCOIN_ROOT", None) is not None:
        print(
            f"warning, your GOLDCOIN_ROOT is set to {os.environ['GOLDCOIN_ROOT']}. "
            f"Please unset the environment variable and run goldcoin init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Goldcoin directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if GOLDCOIN_ROOT is set, or if user has run goldcoin init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
    ...
    print("")
    print("To see your keys, run 'goldcoin keys show --show-mnemonic-seed'")
