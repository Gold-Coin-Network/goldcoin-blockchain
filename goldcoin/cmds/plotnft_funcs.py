...
from goldcoin.cmds.wallet_funcs import print_balance, wallet_coin_unit
from goldcoin.pools.pool_config import load_pool_config, PoolWalletConfig, update_pool_config
...
from goldcoin.util.bech32m import encode_puzzle_hash, decode_puzzle_hash
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojos = uint64(int(fee * units["goldcoin"]))
        ...
        try:
            ...
            while time.time() - start < 10:
                ...
                if len(tx.sent_to) > 0:
                    ...
                    print(f"Do goldcoin wallet get_transaction -f {fingerprint} -tx 0x{tx_record.name} to get status")
                    return None
        except Exception as e:
            print(f"Error creating plot NFT: {e}\n    Please start both farmer and wallet with:  goldcoin start -r farmer")
        return
    print("Aborting.")
