...
from goldcoin.cmds.init_funcs import check_keys, goldcoin_init
from goldcoin.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from goldcoin.daemon.keychain_server import KeychainServer, keychain_commands
from goldcoin.daemon.windows_signal import kill
from goldcoin.plotters.plotters import get_available_plotters
from goldcoin.plotting.util import add_plot_directory
from goldcoin.server.server import ssl_context_for_root, ssl_context_for_server
from goldcoin.ssl.create_ssl import get_mozilla_ca_crt
from goldcoin.util.goldcoin_logging import initialize_logging
...
from goldcoin.util.service_groups import validate_service
from goldcoin.util.setproctitle import setproctitle
from goldcoin.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Goldcoin.")
    quit()
...
service_plotter = "goldcoin_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "goldcoin": "goldcoin",
        "goldcoin_wallet": "start_wallet",
        "goldcoin_full_node": "start_full_node",
        "goldcoin_harvester": "start_harvester",
        "goldcoin_farmer": "start_farmer",
        "goldcoin_introducer": "start_introducer",
        "goldcoin_timelord": "start_timelord",
        "goldcoin_timelord_launcher": "timelord_launcher",
        "goldcoin_full_node_simulator": "start_simulator",
        "goldcoin_seeder": "start_seeder",
        "goldcoin_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if ssl.OPENSSL_VERSION_NUMBER < 0x10101000:
            self.log.warning(
                (
                    ...
                    "A future version of Goldcoin will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
        else:
        ...
        try:
            if Keychain.master_passphrase_is_valid(key, force_reload=True):
                ...
                try:
                    ...
                    self.log.exception("Failed to migrate keys silently. Run `goldcoin keys migrate` manually.")
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["goldcoin", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("goldcoin daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered GOLDCOIN_ROOT
    os.environ["GOLDCOIN_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with GOLDCOIN_ROOT: {os.environ['GOLDCOIN_ROOT']}")
    ...
    if service_command == "goldcoin_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in goldcoin_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    goldcoin_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("goldcoin_daemon")
