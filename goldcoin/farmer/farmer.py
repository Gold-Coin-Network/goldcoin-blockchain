...
from goldcoin.pools.pool_config import PoolWalletConfig, load_pool_config
from goldcoin.protocols import farmer_protocol, harvester_protocol
from goldcoin.protocols.pool_protocol import (
    ErrorResponse,
...
from goldcoin.server.server import ssl_context_for_root
...
from goldcoin.util.bech32m import decode_puzzle_hash
from goldcoin.util.byte_types import hexstr_to_bytes
from goldcoin.util.config import load_config, save_config, config_path_for_filename
from goldcoin.util.hash import std_hash
from goldcoin.util.ints import uint8, uint16, uint32, uint64
...
from goldcoin.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'goldcoin keys generate' or open the UI."
        ...
        if "ozt_target_address" not in self.config:
            self.config = config["farmer"]
        if "ozt_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "ozt_target_address" not in self.config or "ozt_target_address" not in self.pool_config:
            log.debug("ozt_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["ozt_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["ozt_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]):
        ...
        if farmer_target_encoded is not None:
            ...
            config["farmer"]["ozt_target_address"] = farmer_target_encoded
        if pool_target_encoded is not None:
            self.pool_target_encoded = pool_target_encoded
            self.pool_target = decode_puzzle_hash(pool_target_encoded)
            config["pool"]["ozt_target_address"] = pool_target_encoded
    ...
    async def get_cached_harvesters(self, connection: WSGoldcoinConnection) -> HarvesterCacheEntry:
