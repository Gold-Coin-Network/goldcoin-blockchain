...
import goldcoin.server.ws_connection as ws  # lgtm [py/import-and-import-from]
from goldcoin.consensus.block_creation import unfinished_block_to_full_block
from goldcoin.consensus.block_record import BlockRecord
from goldcoin.consensus.blockchain import Blockchain, ReceiveBlockResult
...
from goldcoin.consensus.difficulty_adjustment import get_next_sub_slot_iters_and_difficulty
from goldcoin.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from goldcoin.consensus.multiprocess_validation import PreValidationResult
from goldcoin.consensus.pot_iterations import calculate_sp_iters
from goldcoin.full_node.block_store import BlockStore
from goldcoin.full_node.lock_queue import LockQueue, LockClient
from goldcoin.full_node.bundle_tools import detect_potential_template_generator
from goldcoin.full_node.coin_store import CoinStore
from goldcoin.full_node.full_node_store import FullNodeStore, FullNodeStorePeakResult
from goldcoin.full_node.hint_store import HintStore
from goldcoin.full_node.mempool_manager import MempoolManager
from goldcoin.full_node.signage_point import SignagePoint
from goldcoin.full_node.sync_store import SyncStore
from goldcoin.full_node.weight_proof import WeightProofHandler
from goldcoin.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from goldcoin.protocols.full_node_protocol import (
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
    RespondSignagePoint,
)
from goldcoin.protocols.protocol_message_types import ProtocolMessageTypes
from goldcoin.protocols.wallet_protocol import CoinState, CoinStateUpdate
from goldcoin.server.node_discovery import FullNodePeers
from goldcoin.server.outbound_message import Message, NodeType, make_msg
from goldcoin.server.peer_store_resolver import PeerStoreResolver
...
from goldcoin.types.blockchain_format.pool_target import PoolTarget
...
from goldcoin.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
from goldcoin.types.coin_record import CoinRecord
...
from goldcoin.types.transaction_queue_entry import TransactionQueueEntry
from goldcoin.types.unfinished_block import UnfinishedBlock
from goldcoin.util import cached_bls
from goldcoin.util.bech32m import encode_puzzle_hash
from goldcoin.util.check_fork_next_block import check_fork_next_block
from goldcoin.util.condition_tools import pkm_pairs
from goldcoin.util.config import PEER_DB_PATH_KEY_DEPRECATED
from goldcoin.util.db_wrapper import DBWrapper
from goldcoin.util.errors import ConsensusError, Err, ValidationError
from goldcoin.util.ints import uint8, uint32, uint64, uint128
from goldcoin.util.path import mkdir, path_from_root
from goldcoin.util.safe_cancel_task import cancel_task_safe
from goldcoin.util.profiler import profile_task
from datetime import datetime
from goldcoin.util.db_synchronous import db_synchronous_on
from goldcoin.util.db_version import lookup_db_version
...
class FullNode:
    ...
    def set_server(self, server: GoldcoinServer):
        ...
        elif self.config["port"] == 7999:
            # If `dns_servers` misses from the `config`, hardcode it if we're running mainnet.
            dns_servers.append("dns-introducer.goldcoin-network.net")
        try:
    ...
    async def short_sync_batch(self, peer: ws.WSGoldcoinConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: ws.WSGoldcoinConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ):
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSGoldcoinConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSGoldcoinConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSGoldcoinConnection):
        """
    ...
    def on_disconnect(self, connection: ws.WSGoldcoinConnection):
    ...
    def remove_subscriptions(self, peer: ws.WSGoldcoinConnection):
    ...
    async def sync_from_fork_point(
        ...
        async def fetch_block_batches(batch_queue, peers_with_peak: List[ws.WSGoldcoinConnection]):
            try:
        ...
        batch_queue: asyncio.Queue[Tuple[ws.WSGoldcoinConnection, List[FullBlock]]] = asyncio.Queue(
    ...
    async def update_wallets(
        ...
        for peer, changes in changes_for_peer.items():
            if peer not in self.server.all_connections:
                continue
            ws_peer: ws.WSGoldcoinConnection = self.server.all_connections[peer]
    ...
    async def peak_post_processing(
        ...
        record: BlockRecord,
        fork_height: uint32,
        peer: Optional[ws.WSGoldcoinConnection],
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSGoldcoinConnection] = None,
    ) -> Optional[Message]:
        """
    ...
    async def respond_unfinished_block(
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'ozt')}, "
                f"validation time: {validation_time:0.4f} seconds, {pre_validation_log}"
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSGoldcoinConnection] = None
    ) -> Optional[Message]:
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSGoldcoinConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSGoldcoinConnection):
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSGoldcoinConnection):
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSGoldcoinConnection):
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
...
async def node_next_block_check(
    peer: ws.WSGoldcoinConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
