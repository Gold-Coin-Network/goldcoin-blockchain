...
from goldcoin.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from goldcoin.plotters.chiapos import get_chiapos_install_info, plot_goldcoin
from goldcoin.plotters.madmax import get_madmax_install_info, plot_madmax
from goldcoin.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in GOLDCOIN_ROOT.
    goldcoin_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within GOLDCOIN_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, goldcoin_plotter, "chiapos", "Goldcoinpos Plotter")
    ...
    if args.plotter == "chiapos":
        plot_goldcoin(args, goldcoin_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, goldcoin_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, goldcoin_root_path, root_path)
