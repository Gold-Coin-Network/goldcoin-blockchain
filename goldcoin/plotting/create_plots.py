...
from goldcoin.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from goldcoin.plotting.util import add_plot_directory
from goldcoin.plotting.util import stream_plot_info_ph, stream_plot_info_pk
...
from goldcoin.util.config import config_path_for_filename, load_config
from goldcoin.util.keychain import Keychain
from goldcoin.util.path import mkdir
from goldcoin.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
class PlotKeysResolver:
    ...
    async def get_farmer_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'goldcoin keys add', 'goldcoin keys generate' or provide a public key with -f"
            )
    ...
    async def get_pool_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'goldcoin keys add', 'goldcoin keys generate' or provide a public key with -p"
            )
