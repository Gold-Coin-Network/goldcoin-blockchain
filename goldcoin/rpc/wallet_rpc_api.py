...
from goldcoin.util.keychain import KeyringIsLocked, bytes_to_mnemonic, generate_mnemonic
...
from goldcoin.wallet.derive_keys import master_sk_to_singleton_owner_sk, master_sk_to_wallet_sk_unhardened, MAX_POOL_WALLETS
...
from goldcoin.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
...
class WalletRpcApi:
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("ozt_target_address")
        pool_target = config["pool"].get("ozt_target_address")
        found_farmer = False
        found_pool = False
