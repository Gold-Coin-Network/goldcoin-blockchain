...
from goldcoin.cmds.init_funcs import check_keys
from goldcoin.consensus.block_rewards import calculate_base_farmer_reward
from goldcoin.pools.pool_wallet import PoolWallet
from goldcoin.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
from goldcoin.protocols.protocol_message_types import ProtocolMessageTypes
from goldcoin.server.outbound_message import NodeType, make_msg
...
from goldcoin.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from goldcoin.util.byte_types import hexstr_to_bytes
from goldcoin.util.ints import uint32, uint64
from goldcoin.util.keychain import bytes_to_mnemonic, generate_mnemonic
from goldcoin.util.path import path_from_root
...
from goldcoin.wallet.derive_keys import master_sk_to_singleton_owner_sk
from goldcoin.wallet.rl_wallet.rl_wallet import RLWallet
from goldcoin.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
from goldcoin.wallet.did_wallet.did_wallet import DIDWallet
from goldcoin.wallet.trade_record import TradeRecord
from goldcoin.wallet.transaction_record import TransactionRecord
from goldcoin.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from goldcoin.wallet.util.trade_utils import trade_record_to_dict
...
from goldcoin.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "goldcoin_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "goldcoin_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("cov_target_address")
        pool_target = config["pool"].get("cov_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from goldcoin.pools.pool_wallet_info import initial_pool_state_from_dict
