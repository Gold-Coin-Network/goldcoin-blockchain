...
from goldcoin.cmds.init_funcs import goldcoin_full_version_str
from goldcoin.protocols.protocol_message_types import ProtocolMessageTypes
from goldcoin.protocols.protocol_state_machine import message_response_ok
from goldcoin.protocols.protocol_timing import INTERNAL_PROTOCOL_ERROR_BAN_SECONDS
from goldcoin.protocols.shared_protocol import Capability, Handshake
...
from goldcoin.util.ints import uint8, uint16

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from goldcoin.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSGoldcoinConnection:
    """
    ...
    async def perform_handshake(self, network_id: str, protocol_version: str, server_port: int, local_type: NodeType):
        if self.is_outbound:
            ...
            if inbound_handshake.network_id != 'goldcoin-' + network_id:
