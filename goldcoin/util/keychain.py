...
DEFAULT_USER = f"user-goldcoin-{CURRENT_KEY_VERSION}"  # e.g. user-goldcoin-1.8
DEFAULT_SERVICE = f"goldcoin-{DEFAULT_USER}"  # e.g. goldcoin-user-goldcoin-1.8
...
def supports_keyring_passphrase() -> bool:
    # Support can be disabled by setting GOLDCOIN_PASSPHRASE_SUPPORT to 0/false
    return os.environ.get("GOLDCOIN_PASSPHRASE_SUPPORT", "true").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from goldcoin.cmds.passphrase_funcs import prompt_for_passphrase
