...
# the output will be printed to your goldcoin root path, e.g. ~/.goldcoin/mainnet/profile/
# to analyze the profile, run:

#   python goldcoin/utils/profiler.py ~/.goldcoin/mainnet/profile | less -r

# this will print CPU usage of the goldcoin full node main thread at 1 second increments.
# find a time window of interest and analyze the profile file (which are in pstats format).

# for example:

#   python goldcoin/utils/profiler.py ~/.goldcoin/mainnet/profile 10 20
...
if __name__ == "__main__":
    ...
    def analyze_slot_range(profile_dir: pathlib.Path, first: int, last: int):
        ...
        output_file = "goldcoin-hotspot-%d" % first
        if first < last:
            output_file += "-%d" % last
