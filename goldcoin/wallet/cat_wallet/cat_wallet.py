...
from goldcoin.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict
from goldcoin.util.hash import std_hash
from goldcoin.util.ints import uint8, uint32, uint64, uint128
from goldcoin.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from goldcoin.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
from goldcoin.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
    SpendableCAT,
...
from goldcoin.wallet.derivation_record import DerivationRecord
from goldcoin.wallet.cat_wallet.lineage_store import CATLineageStore
from goldcoin.wallet.lineage_proof import LineageProof
from goldcoin.wallet.payment import Payment
from goldcoin.wallet.puzzles.genesis_checkers import ALL_LIMITATIONS_PROGRAMS
from goldcoin.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
)
from goldcoin.wallet.transaction_record import TransactionRecord
from goldcoin.wallet.util.transaction_type import TransactionType
from goldcoin.wallet.util.wallet_types import WalletType, AmountWithPuzzlehash
from goldcoin.wallet.wallet import Wallet
from goldcoin.wallet.wallet_coin_record import WalletCoinRecord
from goldcoin.wallet.wallet_info import WalletInfo
from goldcoin.wallet.util.compute_memos import compute_memos
import traceback
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            goldcoin_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        goldcoin_tx = dataclasses.replace(goldcoin_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(goldcoin_tx)
    ...
    async def create_tandem_ozt_tx(
        self,
        fee: uint64,
        ...
        if fee > amount_to_claim:
            goldcoin_coins = await self.standard_wallet.select_coins(fee)
            origin_id = list(goldcoin_coins)[0].name()
            goldcoin_tx = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
                (await self.standard_wallet.get_new_puzzlehash()),
                fee=uint64(fee - amount_to_claim),
                coins=goldcoin_coins,
                origin_id=origin_id,  # We specify this so that we know the coin that is making the announcement
                negative_change_allowed=False,
                coin_announcements_to_consume={announcement_to_assert} if announcement_to_assert is not None else None,
            )
            assert goldcoin_tx.spend_bundle is not None

            message = None
            for spend in goldcoin_tx.spend_bundle.coin_spends:
            ...
            goldcoin_coins = await self.standard_wallet.select_coins(fee)
            selected_amount = sum([c.amount for c in goldcoin_coins])
            goldcoin_tx = await self.standard_wallet.generate_signed_transaction(
        ...
        return goldcoin_tx, announcement
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some OZT as part of this
        regular_goldcoin_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_goldcoin_to_claim = payment_amount

        need_goldcoin_transaction = (fee > 0 or regular_goldcoin_to_claim > 0) and (fee - regular_goldcoin_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_goldcoin_transaction:
                    if fee > regular_goldcoin_to_claim:
                        goldcoin_tx, _ = await self.create_tandem_ozt_tx(
                            fee, uint64(regular_goldcoin_to_claim), announcement_to_assert=announcement
                        )
                    ...
                    elif regular_goldcoin_to_claim > fee:
                        goldcoin_tx, _ = await self.create_tandem_ozt_tx(fee, uint64(regular_goldcoin_to_claim))
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        goldcoin_spend_bundle = SpendBundle([], G2Element())
        if goldcoin_tx is not None and goldcoin_tx.spend_bundle is not None:
            goldcoin_spend_bundle = goldcoin_tx.spend_bundle

        return (
            SpendBundle.aggregate(
                [
                    cat_spend_bundle,
                    goldcoin_spend_bundle,
                ]
            ),
            goldcoin_tx,
        )
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, goldcoin_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
        ...
        if goldcoin_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=goldcoin_tx.confirmed_at_height,
                    created_at_time=goldcoin_tx.created_at_time,
                    to_puzzle_hash=goldcoin_tx.to_puzzle_hash,
                    amount=goldcoin_tx.amount,
                    fee_amount=goldcoin_tx.fee_amount,
                    confirmed=goldcoin_tx.confirmed,
                    sent=goldcoin_tx.sent,
                    spend_bundle=None,
                    additions=goldcoin_tx.additions,
                    removals=goldcoin_tx.removals,
                    wallet_id=goldcoin_tx.wallet_id,
                    sent_to=goldcoin_tx.sent_to,
                    trade_id=goldcoin_tx.trade_id,
                    type=goldcoin_tx.type,
                    name=goldcoin_tx.name,
                    memos=[],
                )
            )
