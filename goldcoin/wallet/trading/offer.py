...
from goldcoin.util.bech32m import bech32_encode, bech32_decode, convertbits
from goldcoin.util.ints import uint64
from goldcoin.wallet.util.puzzle_compression import (
...
class Offer:
    ...
    def notarize_payments(
        requested_payments: Dict[Optional[bytes32], List[Payment]],  # `None` means you are requesting OZT
        coins: List[Coin],
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        for asset_id, coins in self.get_offered_coins().items():
            name = "ozt" if asset_id is None else asset_id.hex()
            pending_dict[name] = 0
            for coin in coins:
        ...
        # Then we add a potential fee as pending OZT
        fee: int = sum(c.amount for c in all_removals) - sum(c.amount for c in all_additions)
        if fee > 0:
            pending_dict.setdefault("ozt", 0)
            pending_dict["ozt"] += fee
