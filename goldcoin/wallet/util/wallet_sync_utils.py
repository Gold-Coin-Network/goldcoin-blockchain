...
from goldcoin.types.blockchain_format.coin import hash_coin_list, Coin
...
from goldcoin.util.merkle_set import confirm_not_included_already_hashed, confirm_included_already_hashed, MerkleSet
from goldcoin.wallet.util.peer_request_cache import PeerRequestCache

log = logging.getLogger(__name__)


async def fetch_last_tx_from_peer(height: uint32, peer: WSGoldcoinConnection) -> Optional[HeaderBlock]:
...
async def request_and_validate_removals(
    peer: WSGoldcoinConnection, height: uint32, header_hash: bytes32, coin_name: bytes32, removals_root: bytes32
) -> bool:
...
async def request_and_validate_additions(
    peer: WSGoldcoinConnection, height: uint32, header_hash: bytes32, puzzle_hash: bytes32, additions_root: bytes32
):
...
async def _fetch_header_blocks_inner(
    all_peers: List[WSGoldcoinConnection], selected_peer_node_id: bytes32, request: RequestHeaderBlocks
) -> Optional[RespondHeaderBlocks]:
    if len(all_peers) == 0:
        return None
    random_peer: WSGoldcoinConnection = random.choice(all_peers)
...
async def fetch_header_blocks_in_range(
    ...
    all_peers: List[WSGoldcoinConnection],
