...
from goldcoin.wallet.puzzles.puzzle_utils import (
...
from goldcoin.wallet.secret_key_store import SecretKeyStore
from goldcoin.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_goldcoin(self, goldcoin_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if goldcoin_amount < 0:
            utxos = await self.select_coins(abs(goldcoin_amount), exclude)
        else:
        ...
        goldcoin_amount = spend_value + goldcoin_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(goldcoin_amount), "memos": []}
                ]
