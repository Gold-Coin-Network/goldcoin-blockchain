...
from goldcoin.daemon.keychain_proxy import (
...
from goldcoin.full_node.weight_proof import chunks
from goldcoin.protocols import wallet_protocol
from goldcoin.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from goldcoin.server.node_discovery import WalletPeers
...
from goldcoin.types.weight_proof import WeightProof, SubEpochData
from goldcoin.util.byte_types import hexstr_to_bytes
from goldcoin.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from goldcoin.util.default_root import STANDALONE_ROOT_PATH
from goldcoin.util.ints import uint32, uint64
from goldcoin.util.keychain import KeyringIsLocked, Keychain
from goldcoin.util.path import mkdir, path_from_root
from goldcoin.wallet.util.new_peak_queue import NewPeakQueue, NewPeakQueueTypes, NewPeakItem
from goldcoin.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from goldcoin.wallet.util.wallet_sync_utils import (
...
from goldcoin.wallet.wallet_state_manager import WalletStateManager
from goldcoin.wallet.transaction_record import TransactionRecord
from goldcoin.wallet.wallet_action import WalletAction
...
class WalletNode:
    ...
    server: Optional[GoldcoinServer]
    log: logging.Logger
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'goldcoin keys' program.")
            return None
    ...
    async def _process_new_subscriptions(self):
        while not self._shut_down:
            ...
            peer: Optional[WSGoldcoinConnection] = None
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.goldcoin-network.net"]),
    ...
    def on_disconnect(self, peer: WSGoldcoinConnection):
    ...
    async def on_connect(self, peer: WSGoldcoinConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def long_sync(
        ...
        full_node: WSGoldcoinConnection,
        fork_height: int,
        *,
        rollback: bool,
    ):
    ...
    async def is_peer_synced(
        self, peer: WSGoldcoinConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSGoldcoinConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSGoldcoinConnection]:
        assert self.server is not None
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSGoldcoinConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSGoldcoinConnection):
        if self.wallet_state_manager is None:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSGoldcoinConnection) -> int:
        assert self.wallet_state_manager is not None
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSGoldcoinConnection, peak: HeaderBlock
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSGoldcoinConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
        assert self.wallet_state_manager is not None
        assert self.server is not None
    ...
    async def fetch_puzzle_solution(self, peer: WSGoldcoinConnection, height: uint32, coin: Coin) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSGoldcoinConnection] = None
    ) -> List[CoinState]:
        assert self.server is not None
        all_nodes = self.server.connection_by_type[NodeType.FULL_NODE]
        if len(all_nodes.keys()) == 0:
    ...
    async def fetch_children(
        self, peer: WSGoldcoinConnection, coin_name: bytes32, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
