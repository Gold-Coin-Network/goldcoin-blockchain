...
from goldcoin.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
...
from goldcoin.util.errors import Err, ValidationError
from goldcoin.util.ints import uint32, uint128
from goldcoin.util.keychain import Keychain
from goldcoin.util.lru_cache import LRUCache
from goldcoin.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from goldcoin.wallet.settings.settings_objects import BackupInitialized
...
from goldcoin.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSgoldcoinConnection):
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSgoldcoinConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSgoldcoinConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSgoldcoinConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
