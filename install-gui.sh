...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the goldcoin python virtual environment."
  echo "Execute '. ./activate' before running."
	exit 1
fi

if [ "$(id -u)" = 0 ]; then
  echo "The goldcoin Blockchain GUI can not be installed or run by the root user."
	exit 1
fi

# Allows overriding the branch or commit to build in goldcoin-blockchain-gui
SUBMODULE_BRANCH=$1
...
echo ""
echo "goldcoin blockchain install-gui.sh completed."
echo ""
echo "Type 'cd goldcoin-blockchain-gui' and then 'npm run electron &' to start the GUI."
