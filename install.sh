...
if [ "$(uname -m)" = "armv7l" ]; then
  ...
  echo "https://github.com/Goldcoin-Network/goldcoin-blockchain/wiki/Raspberry-Pi"
  echo "Exiting."
  exit 1
fi
...
install_python3_and_sqlite3_from_source_with_yum() {
  ...
  cd "$TMP_PATH"
  # Install sqlite>=3.37
  # yum install sqlite-devel brings sqlite3.7 which is not compatible with goldcoin
  ...
  # yum install python3 brings Python3.6 which is not supported by goldcoin
  cd ..
      ...
      if [ "$BEST_VERSION" = "" ]; then
        ...
        if [ "$BEST_VERSION" = "3" ]; then
          ...
          if [[ "$PY3_VERSION" =~ 3.10.* ]]; then
            echo "Goldcoin requires Python version <= 3.9.9"
...
echo "Join our Goldcoin community:"
echo "https://discord.gg/HbRxVgjFfa"
echo ""
echo "Earn money while farming Chia & forks"
echo "https://earnapp.com/i/e8ljhze"
echo ""
echo "To install the GUI type 'sh install-gui.sh' after '. ./activate'."
echo ""
echo "Type '. ./activate' and then 'goldcoin init' to begin."
