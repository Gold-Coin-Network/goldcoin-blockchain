...
# Gets the version from the result of "goldcoin version"
...
def get_goldcoin_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["goldcoin", "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_goldcoin_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/goldcoin-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/goldcoin-blockchain-gui/packages/gui/package.json")
