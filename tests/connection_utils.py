...
async def add_dummy_connection(server: goldcoinServer, dummy_port: int) -> Tuple[asyncio.Queue, bytes32]:
    ...
    generate_ca_signed_cert(
        server.goldcoin_ca_crt_path.read_bytes(), server.goldcoin_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.goldcoin_ca_crt_path, server.goldcoin_ca_key_path, dummy_crt_path, dummy_key_path
    )
