...
from goldcoin.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from goldcoin.full_node.full_node_api import FullNodeAPI
from goldcoin.server.start_farmer import service_kwargs_for_farmer
from goldcoin.server.start_full_node import service_kwargs_for_full_node
from goldcoin.server.start_harvester import service_kwargs_for_harvester
from goldcoin.server.start_introducer import service_kwargs_for_introducer
from goldcoin.server.start_service import Service
from goldcoin.server.start_timelord import service_kwargs_for_timelord
from goldcoin.server.start_wallet import service_kwargs_for_wallet
from goldcoin.simulator.start_simulator import service_kwargs_for_full_node_simulator
from goldcoin.timelord.timelord_launcher import kill_processes, spawn_process
...
async def setup_farmer(
    ...
    config["ozt_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "ozt")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["ozt_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "ozt")
