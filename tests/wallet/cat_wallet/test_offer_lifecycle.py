...
from goldcoin.clvm.spend_sim import SpendSim, SimClient
...
from goldcoin.wallet.trading.offer import Offer, NotarizedPayment
...
class TestOfferLifecycle:
    ...
    # This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/OZT coins
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            goldcoin_coins: List[Coin] = all_coins[None]
            red_coins: List[Coin] = all_coins["red"]
            blue_coins: List[Coin] = all_coins["blue"]

            # Create an OZT Offer for RED
            goldcoin_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            goldcoin_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                goldcoin_requested_payments, goldcoin_coins
            )
            goldcoin_announcements: List[Announcement] = Offer.calculate_announcements(goldcoin_requested_payments)
            goldcoin_secured_bundle: SpendBundle = self.generate_secure_bundle(goldcoin_coins, goldcoin_announcements, 1000)
            goldcoin_offer = Offer(goldcoin_requested_payments, goldcoin_secured_bundle)
            assert not goldcoin_offer.is_valid()

            # Create a RED Offer for OZT
            ...
            new_offer = Offer.aggregate([goldcoin_offer, red_offer])
            ...
            # Create yet another offer of BLUE for OZT and RED
            ...
            assert new_offer.summary() == (
                ...
                {"ozt": 900, str_to_tail_hash("red").hex(): 350},
            )
