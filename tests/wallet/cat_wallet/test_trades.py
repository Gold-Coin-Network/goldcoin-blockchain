...
from goldcoin.simulator.simulator_protocol import FarmNewBlockProtocol
...
from goldcoin.wallet.trading.offer import Offer
from goldcoin.wallet.trading.trade_status import TradeStatus
...
class TestCATTrades:
    ...
    async def test_cat_trades(self, wallets_prefarm):
        ...
        MAKER_GOLDCOIN_BALANCE = 20 * 1000000000000 - 100
        TAKER_GOLDCOIN_BALANCE = 20 * 1000000000000 - 100
        await time_out_assert(25, wallet_maker.get_confirmed_balance, MAKER_GOLDCOIN_BALANCE)
        await time_out_assert(25, wallet_taker.get_unconfirmed_balance, TAKER_GOLDCOIN_BALANCE)
        ...
        # goldcoin_for_cat
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(goldcoin_for_cat, fee=uint64(1))
        await asyncio.sleep(1)
        ...
        MAKER_GOLDCOIN_BALANCE -= 2  # -1 and -1 for fee
        MAKER_NEW_CAT_BALANCE += 2
        TAKER_GOLDCOIN_BALANCE += 0  # +1 and -1 for fee
        TAKER_NEW_CAT_BALANCE -= 2

        await time_out_assert(15, wallet_taker.get_unconfirmed_balance, TAKER_GOLDCOIN_BALANCE)
        ...
        await time_out_assert(15, wallet_maker.get_confirmed_balance, MAKER_GOLDCOIN_BALANCE)
        await time_out_assert(15, wallet_maker.get_unconfirmed_balance, MAKER_GOLDCOIN_BALANCE)
        ...
        await time_out_assert(15, wallet_taker.get_confirmed_balance, TAKER_GOLDCOIN_BALANCE)
        await time_out_assert(15, wallet_taker.get_unconfirmed_balance, TAKER_GOLDCOIN_BALANCE)
        ...
        assert len(taker_txs) == 3  # One for each: the outgoing CAT, the incoming goldcoin, and the outgoing goldcoin fee

        # cat_for_goldcoin
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cat_for_goldcoin)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
        ...
        MAKER_CAT_BALANCE -= 4
        MAKER_GOLDCOIN_BALANCE += 3
        TAKER_CAT_BALANCE += 4
        TAKER_GOLDCOIN_BALANCE -= 3
        ...
        assert len(maker_txs) == 1  # The other side will show up as a regular incoming transaction
        assert len(taker_txs) == 2  # One for each: the outgoing goldcoin, the incoming CAT
        ...
        # goldcoin_for_multiple_cat
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(goldcoin_for_multiple_cat)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
        ...
        MAKER_GOLDCOIN_BALANCE -= 7
        MAKER_CAT_BALANCE += 8
        MAKER_NEW_CAT_BALANCE += 9
        TAKER_GOLDCOIN_BALANCE += 7
        TAKER_CAT_BALANCE -= 8
        ...
        # multiple_cat_for_goldcoin
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(multiple_cat_for_goldcoin)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
        ...
        MAKER_GOLDCOIN_BALANCE += 10
        TAKER_CAT_BALANCE += 11
        TAKER_NEW_CAT_BALANCE += 12
        TAKER_GOLDCOIN_BALANCE -= 10
        ...
        # goldcoin_and_cat_for_cat
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(goldcoin_and_cat_for_cat)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
        ...
        MAKER_GOLDCOIN_BALANCE -= 13
        MAKER_CAT_BALANCE -= 14
        MAKER_NEW_CAT_BALANCE += 15
        TAKER_GOLDCOIN_BALANCE += 13
    ...
    async def test_trade_cancellation(self, wallets_prefarm):
        ...
        MAKER_GOLDCOIN_BALANCE = 20 * 1000000000000 - 100
        MAKER_CAT_BALANCE = 100
        TAKER_GOLDCOIN_BALANCE = 20 * 1000000000000
        await time_out_assert(15, wallet_maker.get_confirmed_balance, MAKER_GOLDCOIN_BALANCE)
        ...
        await time_out_assert(15, wallet_maker.get_confirmed_balance, MAKER_GOLDCOIN_BALANCE - FEE)
        await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, MAKER_CAT_BALANCE)
        await time_out_assert(15, wallet_taker.get_confirmed_balance, TAKER_GOLDCOIN_BALANCE)
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(goldcoin_for_cat)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
