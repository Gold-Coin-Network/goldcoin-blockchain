...
from goldcoin.rpc.full_node_rpc_api import FullNodeRpcApi
from goldcoin.rpc.full_node_rpc_client import FullNodeRpcClient
from goldcoin.rpc.rpc_server import start_rpc_server
from goldcoin.rpc.wallet_rpc_api import WalletRpcApi
...
from goldcoin.wallet.transaction_sorting import SortKey
...
class TestWalletRpc:
    ...
    async def test_wallet_rpc(self, two_wallet_nodes, trusted):
        ...
        try:
            ...
            addr = encode_puzzle_hash(await wallet_node_2.wallet_state_manager.main_wallet.get_new_puzzlehash(), "ozt")
            tx_amount = 15600000
            try:
            ...
            for i in range(0, 5):
                await client.farm_block(encode_puzzle_hash(ph_2, "ozt"))
            ...
            # Test RELEVANCE
            await client.send_transaction("1", 1, encode_puzzle_hash(ph_2, "ozt"))  # Create a pending tx
            ...
            # Create an offer of 5 goldcoin for one CAT
            ...
            assert summary == {"offered": {"ozt": 5}, "requested": {col.hex(): 1}}
            ...
            test_config["farmer"]["ozt_target_address"] = encode_puzzle_hash(test_ph, "tozt")
            # set pool to second private key
            sk = await wallet_node.get_key_for_fingerprint(pks[1])
            test_ph = create_puzzlehash_for_pk(master_sk_to_wallet_sk(sk, uint32(0)).get_g1())
            test_config["pool"]["ozt_target_address"] = encode_puzzle_hash(test_ph, "tozt")
